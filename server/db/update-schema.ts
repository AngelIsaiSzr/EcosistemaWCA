...
const updateSchema = async () => {
  ...
  try {
    ...
    await runQuery(
      client,
      "columna enrollments.updated_at",
      `ALTER TABLE enrollments ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;`,
    );
    await runQuery(
      client,
      "backfill enrollments.updated_at",
      `UPDATE enrollments SET updated_at = COALESCE(updated_at, created_at, CURRENT_TIMESTAMP) WHERE updated_at IS NULL;`,
    );
    await runQuery(
      client,
      "tabla certificates",
    ...
    await runQuery(
      client,
      "índice certificates user+course",
      `CREATE UNIQUE INDEX IF NOT EXISTS certificates_user_course_unique ON certificates (user_id, course_id);`,
    );
    await runQuery(
      client,
      "índice certificates code",
